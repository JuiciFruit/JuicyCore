...
public class NormalEffectData extends ParticleEffectData {
	...
	public NormalEffectData(ParticleEffect effect, float offsetX,
			float offsetY, float offsetZ, int amount, float speed) {
		super(offsetX, offsetY, offsetZ, amount);
		this.effect = effect;
		this.speed = speed;
	}

	public static NormalEffectData fromString(String s)
		...
		if (effect == null)
			throw new IllegalArgumentException(
					"Contains an invalid particle effect name");
		return new NormalEffectData(effect, Float.parseFloat(p[1]),
