package juicydev.jcore.utils.midi;
...
public class NoteBlockReceiver implements Receiver {
	...
	public NoteBlockReceiver(Location loc) throws InvalidMidiDataException,
			IOException {
		this.listeners = null;
		this.globalLoc = loc;
	}
	...
	public void playNote(ShortMessage message) {
		...
		if (globalLoc != null) {
			globalLoc.getWorld().playSound(globalLoc, Sound.NOTE_PIANO, volume,
					pitch);
		} else if (listeners != null) {
			for (Player player : listeners)
				player.playSound(player.getLocation(), Sound.NOTE_PIANO,
						volume, pitch);
		}
	}
