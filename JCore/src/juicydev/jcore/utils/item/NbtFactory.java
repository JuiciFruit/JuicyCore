...
public class NbtFactory {
	...
	private NbtFactory() {
		if (BASE_CLASS == null) {
			try {
				...
				LOAD_COMPOUND = READ_LIMITER_CLASS != null ? new LoadMethodSkinUpdate(
						STREAM_TOOLS, READ_LIMITER_CLASS)
						: new LoadMethodWorldUpdate(STREAM_TOOLS);
				SAVE_COMPOUND = getMethod(Modifier.STATIC, 0, STREAM_TOOLS,
						null, BASE_CLASS, DataOutput.class);
	...
	private void initializeNMS(ClassLoader loader, String nmsPackage) {
		try {
			STREAM_TOOLS = loader.loadClass(nmsPackage
					+ ".NBTCompressedStreamTools");
			READ_LIMITER_CLASS = loader.loadClass(nmsPackage
					+ ".NBTReadLimiter");
		} catch (ClassNotFoundException e) {
	...
	private String getPackageName() {
		...
		String name = server != null ? server.getClass().getPackage().getName()
				: null;
	...
	private static class LoadMethodWorldUpdate extends LoadCompoundMethod {
		public LoadMethodWorldUpdate(Class<?> streamClass) {
			setMethod(getMethod(Modifier.STATIC, 0, streamClass, null,
					DataInput.class));
		}
	...
	private static class LoadMethodSkinUpdate extends LoadCompoundMethod {
		...
		public LoadMethodSkinUpdate(Class<?> streamClass,
			...
			setMethod(getMethod(Modifier.STATIC, 0, streamClass, null,
					DataInput.class, readLimiterClass));
				...
				if (readLimiterClass.isAssignableFrom(field.getType())) {
					try {
						...
						throw new RuntimeException(
								"Cannot retrieve read limiter.", e);
					}
				}
			}
		}
