...
public class BlockDustData extends ParticleEffectData {
	...
	public BlockDustData(int id, byte data, float offsetX, float offsetY,
			float offsetZ, int amount, float speed) {
	...
	public static BlockDustData fromString(String s)
			throws IllegalArgumentException {
		if (!s.matches(FORMAT))
			throw new IllegalArgumentException("Invalid format");
		String[] p = s.split("@");
		return new BlockDustData(Integer.parseInt(p[0]), Byte.parseByte(p[1]),
				Float.parseFloat(p[2]), Float.parseFloat(p[3]),
				Float.parseFloat(p[4]), Integer.parseInt(p[5]),
				Float.parseFloat(p[6]));
	}

	@Override
	public void displayEffect(Location center, Player... players) {
		ParticleEffect.displayBlockDust(center, id, data, offsetX, offsetY,
				offsetZ, speed, amount, players);
	}
	...
	public void displayEffect(Location center, double range) {
		ParticleEffect.displayBlockDust(center, range, id, data, offsetX,
